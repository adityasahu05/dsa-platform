          ...
          {['questions', 'submissions', 'analytics'].map(tab => (
            ...
            >
              {tab === 'questions' ? `Questions (${questions.length})` :
               tab === 'submissions' ? `Submissions (${Array.isArray(detailedAnalytics?.students) ? detailedAnalytics.students.length : 0})` : 'Analytics'}
            </button>
          ))}
        </div>
      </div>
          ...
          <>
            ...
            {activeTab === 'submissions' && (
              <div style={{ backgroundColor: '#fff', border: '1px solid #e0e0e0', borderRadius: '4px', overflow: 'hidden' }}>
                {(!detailedAnalytics || !Array.isArray(detailedAnalytics.students) || detailedAnalytics.students.length === 0) ? (
