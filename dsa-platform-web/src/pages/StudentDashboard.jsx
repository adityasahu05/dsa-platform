...
export default function StudentDashboard() {
  ...
  const handleFindTest = async () => {
    ...
    setTestPreview(null);
    try {
      const res = await fetch(`${API_BASE}/api/teacher/student/test/lookup/${code}`);
      if (!res.ok) {
        setError("No test found with that code. Please check and try again.");
      } else {
  ...
  const handleBeginTest = () => {
    if (!testPreview) return;
    navigate("/match", { state: { test: testPreview } });
  };
  ...
  return (
    <div className="sd-root">
      <style>{`
        ...
        .sd-root {
          min-height: 100vh;
          background: radial-gradient(circle at top, rgba(37,99,235,0.12), transparent 55%), #0b0f19;
          color: #e5e7eb;
        ...
        @media (max-width: 1100px) {
          ...
          .sd-left { border-right: none; border-bottom: 1px solid rgba(148,163,184,0.2); }
        }
      ...
      <div className="sd-header">
        ...
        <span className="sd-title">SlashCoder</span>
      </div>

      <div className="sd-grid">
        <div className="sd-left">
          <div className="sd-heading">Practice Programming</div>
          <div className="sd-sub">Choose your language and start solving structured problems</div>
          ...
          <div className="sd-cards">
            {LANGUAGES.map((lang) => (
              <button
                ...
                className="sd-card"
              >
                <div style={{ width: 56, height: 56, borderRadius: 14, background: lang.iconBg, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 14 }}>
                  {lang.icon}
                </div>
                <div className="sd-card-title">{lang.label}</div>
                <div className="sd-card-desc">{lang.desc}</div>
                <div className="sd-card-footer">
                  <span>Beginner Path</span>
                  <span style={{ color: lang.color, fontWeight: 600 }}>Start ?</span>
                </div>
              </button>
            ))}
          </div>
        ...
        <div className="sd-right">
          <div style={{ fontSize: 20, fontWeight: 700, marginBottom: 6 }}>Attempt a Test</div>
          <div className="sd-muted" style={{ marginBottom: 18 }}>Enter the test ID shared by your teacher</div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            ...
            <button
              ...
              onClick={handleFindTest}
              disabled={loading || !testCode.trim()}
              style={{ opacity: loading || !testCode.trim() ? 0.6 : 1 }}
            >
              {loading ? "..." : "Find"}
            </button>
          </div>
          ...
          {error && (
            <div className="sd-panel" style={{ borderColor: "rgba(248,113,113,0.5)", background: "rgba(127,29,29,0.25)" }}>
              <div style={{ color: "#fca5a5", fontSize: 13 }}>{error}</div>
            </div>
          )}

          {testPreview && (
            <div className="sd-panel" style={{ borderColor: "rgba(37,99,235,0.4)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 12 }}>
                <div>
                  <div style={{ fontWeight: 700, fontSize: 16 }}>{testPreview.title}</div>
                  {testPreview.description && (
                    <div className="sd-muted" style={{ marginTop: 4 }}>{testPreview.description}</div>
                  )}
                </div>
                <span className="sd-muted" style={{ background: "rgba(59,130,246,0.2)", color: "#93c5fd", padding: "4px 10px", borderRadius: 999 }}>
                  Found
                </span>
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 8, marginBottom: 14 }}>
                {testPreview.duration_minutes && (
                  <div className="sd-panel" style={{ margin: 0, padding: 10, textAlign: "center" }}>
                    <div style={{ fontWeight: 700, fontSize: 16 }}>{testPreview.duration_minutes}</div>
                    <div className="sd-muted">Minutes</div>
                  </div>
                )}
                {testPreview.question_count && (
                  <div className="sd-panel" style={{ margin: 0, padding: 10, textAlign: "center" }}>
                    <div style={{ fontWeight: 700, fontSize: 16 }}>{testPreview.question_count}</div>
                    <div className="sd-muted">Questions</div>
                  </div>
                )}
                {testPreview.allowed_languages && (
                  <div className="sd-panel" style={{ margin: 0, padding: 10, textAlign: "center" }}>
                    <div style={{ fontWeight: 700, fontSize: 14, textTransform: "capitalize" }}>{testPreview.allowed_languages[0]}</div>
                    <div className="sd-muted">Language</div>
                  </div>
                )}
              </div>
          ...
          {!testPreview && !error && (
            <div className="sd-panel" style={{ borderStyle: "dashed", textAlign: "center", color: "rgba(226,232,240,0.5)" }}>
              Enter a test ID above to get started
            </div>
          )}
        </div>
